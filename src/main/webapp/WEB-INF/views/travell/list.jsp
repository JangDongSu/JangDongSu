...
<div class="row">
	<div class="searchform">
		<form action="/travell/list" method="get" id="searchForm">
	...
	<div class="gallery-list">
		<ul>
			<c:forEach items="${list}" var="travell">
            	<li>
            		<a href="<c:out value="${travell.t_bno}"/>" class="move">
            			<span class="gal-area"><c:out value="${travell.area}"/>/<c:out value="${travell.kind}"/></span>
            			<span class="view-image" id='<c:out value="${travell.t_bno}"/>'>
            			</span>	
                   		<span class="gallery-title">
                       		<span class="gal-object"><c:out value="${travell.title}"/>&nbsp;<b>[<c:out value="${travell.replyCnt}"/>]</b></span>
                       		<span class="gal-writer"><c:out value="${travell.writer}"/></span> 
	...
	<div class="col-lg-12">
		...
		<form action="/travell/list" method="get" id="actionForm">
...
<script type="text/javascript">
    $(document).ready(function() {
    	...
    	$("#regBtn").on("click", function() {
    		self.location = "/travell/register";
    	});
    	...
    	$(".paginate_button a").on("click", function(e){
    		...
    		actionForm.attr("action", "/travell/list");
    		actionForm.submit();
    	});
    	...
    	$(".move").on("click", function(e){
    		e.preventDefault();
    		if(actionForm.find("input[name='t_bno']").val()!=''){
    			actionForm.find("input[name='t_bno']").remove();
    		}
    		actionForm.append("<input type='hidden' name='t_bno' value='" + $(this).attr("href") + "'>");
    		actionForm.attr("action", "/travell/get");
    		actionForm.submit();
    	});
