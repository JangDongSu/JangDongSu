	...
	<div class="container">
		<div class="bo-table">
			<table class="table table-hover">
				...
				<c:forEach items="${notice}" var="notice">
					<tr>
						<td><c:out value="${notice.n_bno}" /></td>
						<td><%--<a href='<c:out value="${notice.n_bno}"/>' class="move"> <c:out value="${notice.n_title}" /></a> --%>
						<c:out value="${notice.n_title}" /></td>
						<td><c:out value="${notice.n_writer}" /></td>
		...
		<div class="searchform">
			<form action="/notice/list" method="get" id="searchForm">
				...
				<input type="text" name="keyword" class="select-style" value="<c:out value="${pageMaker.criteria.keyword}"/>" /> 
			...
			<button id="regBtn" type="button" class="button" style="float: right;">게시물 작성</button>
			</sec:authorize>
		</div>
		<div class="page-wrap">
			<ul class="page">
...
<script type="text/javascript">
	...
	$(".move").on("click",function(e) {
		e.preventDefault();
		/*if(actionForm.find("input[name='n_bno']").val()!=''){
			actionForm.find("input[name='n_bno']").remove();
			}*/
